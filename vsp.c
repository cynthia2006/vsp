...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < NUM_POINTS; ++i)
        {
            ...
            const int lo_bin = fc * BIN_WIDTH, // floorf(fc * BIN_WIDTH)
                      hi_bin = ceilf(fnext * BIN_WIDTH);

            // Highest bin represents the band.
            float mag_max = 0.0;
            for (int bi = lo_bin; bi < hi_bin; ++bi)
            {
                const kiss_fft_cpx bin = freq_bins[bi];
                const float mag = FFT_SCALE * hypotf(bin.r, bin.i);

                mag_max = fmaxf(mag_max, mag);
            }

            // Exponential smoothing to make animation smoother.
            sm_freqs[i] = sm_freqs[i] * state.tau + (1.0 - state.tau) * mag_max;
